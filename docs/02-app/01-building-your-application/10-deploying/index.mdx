...
- [Configuring self-hosted Next.js behind firewalls and load balancers](#configuring-self-hosted-nextjs-behind-firewalls-and-load-balancers)
...
### Configuring self-hosted Next.js behind firewalls and load balancers

When self-hosting Next.js behind firewalls, load balancers, or caching layers, ensure full functionality of features like `next/link` and React Server Components (RSC) by allowing the following headers through to your Next.js application:
...
Consider these headers when defining cache keys for unique requests. Note that navigation between pages with RSC and `next/link` uses the `fetch` API, including a query parameter `_rsc` (e.g., `GET /new-page?_rsc=1234`). Decide if `_rsc` should be part of your cache key, which may result in each page navigation being treated as a unique request, thus bypassing the cache.
